...
function initDates(){ $('#otDetailMonth').value=$('#otDetailMonth').value||ym(); $('#otConfirmMonth').value=$('#otConfirmMonth').value||ym(); $('#cardMonth').value=$('#cardMonth').value||ym(); $('#vehMonth').value=$('#vehMonth').value||ym(); $('#weekScope').value=$('#weekScope').value||'2026-06-22'; }
function renderAll(){ if(document.activeElement && ['INPUT','TEXTAREA','SELECT'].includes(document.activeElement.tagName)) return; renderWeekly(); renderOvertimeDetail(); renderOvertimeConfirm(); renderCards(); renderVehicle(); }
...
function overtimeTableHTML(s){ const data=(DB.overtime[s]||[]).sort((a,b)=>(a.workDate||'').localeCompare(b.workDate||'') || (a.name||'').localeCompare(b.name||'')); return `<table class="grid overtime-grid"><thead><tr>${['근무처','직위','성명','근무일','시작','종료','휴게(분)','시간','내용','근무상황부','사전명령서','확인대장','비고','삭제'].map(h=>`<th>${h}</th>`).join('')}</tr></thead><tbody>${data.map(r=>`<tr><td>${inputCell('overtime',s,r.id,'workplace',r.workplace||r.department||'')}</td><td>${inputCell('overtime',s,r.id,'position',r.position||'')}</td><td>${inputCell('overtime',s,r.id,'name',r.name)}</td><td>${inputCell('overtime',s,r.id,'workDate',r.workDate,'date')}</td><td>${inputCell('overtime',s,r.id,'startTime',r.startTime,'time')}</td><td>${inputCell('overtime',s,r.id,'endTime',r.endTime,'time')}</td><td>${inputCell('overtime',s,r.id,'breakMinutes',r.breakMinutes||0,'number')}</td><td>${calcHours(r)}</td><td>${inputCell('overtime',s,r.id,'reason',r.reason,'textarea')}</td><td>${inputCell('overtime',s,r.id,'statusSubmit',r.statusSubmit||'완료','select',['','완료','미제출','해당없음'])}</td><td>${inputCell('overtime',s,r.id,'orderSubmit',r.orderSubmit||'완료','select',['','완료','미제출','해당없음'])}</td><td>${inputCell('overtime',s,r.id,'ledgerComplete',r.ledgerComplete||'완료','select',['','완료','미작성','해당없음'])}</td><td>${inputCell('overtime',s,r.id,'note',r.note,'textarea')}</td><td><button class="del" data-t="overtime" data-s="${s}" data-id="${r.id}">삭제</button></td></tr>`).join('')}</tbody></table>`; }
function renderOvertimeDetail(){ const s=$('#otDetailMonth').value; $('#overtimeDetailTable').innerHTML=overtimeTableHTML(s); bindGrid(); }
function renderOvertimeConfirm(){ const s=$('#otConfirmMonth').value; $('#overtimeConfirmTable').innerHTML=overtimeTableHTML(s); bindGrid(); }
...
$('#addOvertimeDetail').onclick=()=>add('overtime',$('#otDetailMonth').value); $('#addOvertimeConfirm').onclick=()=>add('overtime',$('#otConfirmMonth').value); $('#addCard').onclick=()=>add('cards',$('#cardMonth').value); $('#addVehicle').onclick=()=>add('vehicle',$('#vehMonth').value);
$('#printWeekly').onclick=async()=>{await saveNow(); window.open(`/print/weekly?scope=${$('#weekScope').value}`,'_blank');};
$('#downloadOvertimeDetail').onclick=async()=>{await saveNow(); location.href=`/download/overtime-schedule.xlsx?month=${$('#otDetailMonth').value}`;}; $('#downloadOvertimeConfirm').onclick=async()=>{await saveNow(); location.href=`/download/overtime.xlsx?month=${$('#otConfirmMonth').value}`;}; $('#downloadCards').onclick=async()=>{await saveNow(); location.href=`/download/cards.xlsx?month=${$('#cardMonth').value}`;}; $('#downloadVehicle').onclick=async()=>{await saveNow(); location.href=`/download/vehicle.xlsx?month=${$('#vehMonth').value}`;};
['otDetailMonth','otConfirmMonth','cardMonth','vehMonth'].forEach(id=>$('#'+id).onchange=renderAll);
